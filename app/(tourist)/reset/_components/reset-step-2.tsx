...
import { verifyCaptcha, verifyResetAccount, sendMail } from "@/lib/api/auth";
...
export default function ResetStep2({
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    verifyCaptcha(ticket, captcha, "reset")
      .then((res) => {
  ...
  const handleResend = async () => {
    const res = await verifyResetAccount(username);
    if (res.data.Success) {
      const newTicket = res.data.Data.resetPwdTicket;
      onTicket(newTicket);
      await sendMail(newTicket, "reset");
    }
  };
