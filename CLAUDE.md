...
- Web mode: `pnpm dev`
- Desktop mode: `pnpm tauri dev`

Tauri production builds depend on the static export generated by `pnpm build`.
...
- `src-tauri/tauri.conf.json` points `frontendDist` to `../out`
- `beforeDevCommand` runs `pnpm dev`
- `beforeBuildCommand` runs `pnpm build`
- `src-tauri/src/lib.rs` enables log plugin in debug builds
...
- The npm package name and some Tauri metadata still use starter-style naming; treat current code/config as source of truth rather than marketing labels.
